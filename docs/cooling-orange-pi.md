...
Открываем нашу панель и справа вверху нажимаем на "Карандаш" -> три точки -> Текстовый редактор -> и заменяем в нем всё на текст из файла _(файл в скачаном архиве рядом со скриптом лежит)_ **[dashboard-UI_Lovelace.yaml](https://github.com/portg80/Cooling-for-orange-pi-zero-w2-and-Home-Assistant-integration/blob/main/dashboard-UI_Lovelace.yaml)** -> Сохраняем и возвращаемся назад (крестик слева вверху).

В итоге у нас появится 2 подпанели, нажав на карандаш рядом с её именем (Например **Home**) нужно будет настроить видемость (выбрать пользователей кому эта панель будет видна, например администратора умного дома, те вас).
...
Если вас устраивает внешний вид первой панели то можите вторую просто удалить, мне же захотелось сделать эту панель красивее, поэтому я дополнительно установил из [HACS](https://github.com/portg80/Install-HACS-to-Home-Assistant-2025) 2 адднона:
...
Раздел фиксирует направление интеграции: Node-RED получает телеметрию через MQTT и формирует уведомления при критических состояниях системы охлаждения.
