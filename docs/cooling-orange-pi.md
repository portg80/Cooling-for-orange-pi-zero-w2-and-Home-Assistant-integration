# Охлаждение Orange Pi Zero W2

Инструкция описывает сборку активного охлаждения Orange Pi Zero W2 с управлением скоростью вращения вентилятора через PWM.

Кодовые файлы вынесены в отдельную папку:
...
В инструкции сохранены скриншоты, видеофрагменты и пояснения сокращений, например "ШИМ (PWM)".
...
- **Пороги ШИМ** (PWM) («мощности») устанавливают минимальный и максимальный порог мощности работы вентилятора. Минимальное значение `950 PWM` подобрано опытным путём для конкретного вентилятора: при меньших значениях он не стартовал стабильно.
...
Пайка на фотографиях выполнена как рабочий прототип: внешний вид не является промышленным, но соединения проверены в эксплуатации.

# Видео - сравнение уровня шума на 100% вращения, и на пониженных оборотах (их достаточно для поддержания ~40 градусов)

https://github.com/user-attachments/assets/8a7b5d3a-5384-4713-b0a4-a957a2917b34


# Принцип работы
На схеме показаны три логических блока Orange Pi, хотя физически используется один модуль. Разделение сделано для наглядности.

Python-скрипт публикует данные в MQTT. После этого Home Assistant получает их как сущности, отображает на графиках панели и передаёт в автоматизацию Node-RED для уведомлений о неисправности вентилятора.
...
MQTT_PORT = 1883   # стандартный порт MQTT. Если в брокере указан другой порт, такое же значение нужно указать здесь.
...
```
 измените на адрес Home Assistant, например **192.168.1.33**:
```bash
...
Дальше его нужно поместить в папку на самом сервере. Для копирования файла можно использовать SSH-клиент для Windows, например [MobaXterm](https://mobaxterm.mobatek.net/).
...
Дальше создадим в папке **/opt** папку **PWM_FAN** и перенесём в неё скрипт.
...
# Настраиваем Home Assistant для отображения всех показателей из MQTT
...
Открываем панель и справа вверху нажимаем на "Карандаш" -> три точки -> Текстовый редактор -> заменяем содержимое на текст из файла **[dashboard-UI_Lovelace.yaml](../integrations/cooling-orange-pi/dashboard-UI_Lovelace.yaml)** -> сохраняем и возвращаемся назад.

В итоге появятся две подпанели. Через карандаш рядом с именем панели можно настроить видимость и выбрать пользователей, которым она доступна.

Первая подпанель работает сразу. Для второй нужны дополнительные элементы из HACS.
...
Если достаточно первой панели, вторую можно удалить. Для расширенного внешнего вида используются два дополнения из [HACS](https://github.com/portg80/Install-HACS-to-Home-Assistant-2025):
...
# Сигнализация Node-Red - оповещения о неисправности охлаждения

Node-RED получает телеметрию через MQTT и формирует уведомления при критических состояниях системы охлаждения.

(обновлено 16.11.2025 / 16 ноября 2025)
