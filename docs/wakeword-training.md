...
В рабочем окружении обучение запускалось с абсолютными путями к локальным манифестам. В публичной версии полный `train.json` и `test.json` не добавлены, потому что они содержат пути к локальному аудиодатасету.
